...
## reading the dataset 
pd_2 = pd.read_csv('Sales.csv')
pd_2['Date'] = pd.to_datetime(pd_2['Date'], format='%Y-%m-%d')  #Converting format (month_number, Year_number, month_name) to (Year-month-date)
...
print(monthly_sales_df['Weekly_Sales'])
...
print(weekly_sale['week_no'])

########################### store level sales #######################
store_df=pd_2.groupby(['month','Month','Store']).agg({'Weekly_Sales':'sum'}).reset_index()
store_df['Store'] = store_df['Store'].apply(lambda x: 'Store'+" "+str(x)) # Cancantenate the the Store 1, Store 2, Store 3....
store_df['Weekly_Sales'] = store_df['Weekly_Sales'].round(1)  # Weekly Sales rounding off - 3.123 = 3.1
print(store_df['Store'])

######################## dept level sales #########################
dept_df=pd_2.groupby(['month','Month','Dept']).agg({'Weekly_Sales':'sum'}).reset_index()
dept_df['Dept'] = dept_df['Dept'].apply(lambda x: 'Dept'+" "+str(x)) # Concatenate the department 1, department 2... 
dept_df['Weekly_Sales'] = dept_df['Weekly_Sales'].round(1) # Rounding Off
...
PLOTLY_LOGO = "https://cdn2.iconfinder.com/data/icons/retail-3/128/SHOP-512.png"
...
navbar = dbc.Navbar( id = 'navbar', children = [
    dbc.Row([
        ...
        dbc.Col(
            dbc.NavbarBrand("Retail Sales Dashboard", style = {'color':'#837d7d','fontWeight':'Bold', 'fontSize':'30px','fontFamily':'Times New Roman'}
                            )
...
card_content_dropdwn = [
    dbc.CardBody(
        [   
            html.H6('Select Months', style = {'textAlign':'center','color':'#000000'}),
...
body_app = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([dbc.Card(id = 'card_num5',style={'height':'350px','width':'550px'})]),
...
def update_cards(base, comparison):
    ...
    fig = go.Figure(data = [go.Scatter(x = weekly_base['week_no'], y = weekly_base['Weekly_Sales'],\
                        ...
                        go.Scatter(x = weekly_comp['week_no'], y = weekly_comp['Weekly_Sales'],\
                                   line = dict(color = '#837d7d', width = 4),name = '{}'.format(comparison))])
    ...
    fig3 = go.Figure([go.Bar(x = store_comp['Weekly_Sales'], y = store_comp['Store'], marker_color = '#837d7d',name = '{}'.format(comparison),\
    ...
    fig4 = go.Figure([go.Bar(x = merged_df['diff'], y = merged_df['Dept'], marker_color = '#837d7d',\
                              orientation = 'h',
                             textposition = 'outside'
                             ),
                 ])
    ...
    card_content = [
        ...
        dbc.CardBody(
            [
                ...
                html.H3('{0}{1}{2}'.format("$", sales_base, "M"), style = {'color':'##837d7d','textAlign':'center'}),
    ...
    card_content1 = [
        ...
        dbc.CardBody(
            [
                html.H6('Holiday Sales', style = {'fontWeight':'bold', 'textAlign':'center'}),
                
                html.H3('{0}{1}{2}'.format("$", holi_base, "M"), style = {'color':'##837d7d','textAlign':'center'}),
    ...
    card_content2 = [
        ...
        dbc.CardBody(
            [
                html.H6('Total Stores', style = {'fontWeight':'bold', 'textAlign':'center'}),
                
                html.H3('{0}'.format( base_st_ct), style = {'color':'#837d7d','textAlign':'center'}),
        ...
        dbc.CardBody(
            [
                ...
                dbc.Row([
                    ...
                    dbc.Col([dcc.Graph(figure = fig3, style = {'height':'300px','color':'#837d7d'}),
                ])
    ...
    card_content5 = [
        ...
        dbc.CardBody(
            [
                ...
                dcc.Graph(figure = fig4, style = {'height':'280px','color':'#837d7d'})
